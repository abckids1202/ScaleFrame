...
export const comparisons = sqliteTable("comparisons", {
  id: id("id"), slug: text("slug").notNull(), ownerAccountId: text("owner_account_id").notNull(), title: text("title").notNull(), domain: text("domain").notNull(), status: text("status").notNull().default("draft"), formatMode: text("format_mode").notNull().default("decisive"), matchupType: text("matchup_type").notNull().default("character"), participantCountA: integer("participant_count_a").notNull().default(1), participantCountB: integer("participant_count_b").notNull().default(1), scoreMode: text("score_mode").notNull().default("scaled"), spoilerLevel: integer("spoiler_level").notNull().default(0), contentRating: text("content_rating").notNull().default("general"), difficulty: text("difficulty").notNull().default("high_diff"), overallMethod: text("overall_method").notNull().default("normalized_weighted_score"), rulesJson: text("rules_json").notNull().default("{}"), revisionNumber: integer("revision_number").notNull().default(1), createdAt: createdAt(), updatedAt: updatedAt(), publishedAt: text("published_at"),
...
export const comparisonCollaborationRequests = sqliteTable("comparison_collaboration_requests", {
  id: id("id"), comparisonId: text("comparison_id").notNull(), requesterAccountId: text("requester_account_id").notNull(), targetAccountId: text("target_account_id"), targetHandle: text("target_handle"), message: text("message").notNull().default(""), status: text("status").notNull().default("pending"), responseNote: text("response_note").notNull().default(""), createdAt: createdAt(), updatedAt: updatedAt(), resolvedAt: text("resolved_at"),
}, (table) => ({ comparisonIndex: index("idx_comparison_collab_comparison_status").on(table.comparisonId, table.status), requesterIndex: index("idx_comparison_collab_requester").on(table.requesterAccountId), targetIndex: index("idx_comparison_collab_target").on(table.targetAccountId, table.status) }));
