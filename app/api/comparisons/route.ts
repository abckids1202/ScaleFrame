...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { title, domain, formatMode, matchupType, participantCountA, participantCountB, scoreMode, spoilerLevel, contentRating } = parsed.data;
    const record = { id: newId("cmp"), slug: `${slugify(title)}-${crypto.randomUUID().slice(0, 6)}`, ownerAccountId: user.subject, title, domain, status: "draft", formatMode, matchupType, participantCountA, participantCountB, scoreMode, spoilerLevel, contentRating, difficulty: "high_diff", overallMethod: "normalized_weighted_score", rulesJson: "{}", revisionNumber: 0 } as const;
