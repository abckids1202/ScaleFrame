...
export async function POST(request: Request) {
  ...
  try { const { client, responseHeaders } = createRouteSupabaseClient(request); const { data, error } = await client.auth.signInWithPassword({ email, password }); if (error || !data.user) return errorResponse("Email or password is incorrect.", 401, "invalid_credentials"); await syncAccount({ id: data.user.id, email: data.user.email, email_confirmed_at: data.user.email_confirmed_at }); const response = jsonResponse({ user: { id: data.user.id, email: data.user.email, emailVerified: Boolean(data.user.email_confirmed_at) }, authenticated: true }); return applySupabaseHeaders(response, responseHeaders); }
